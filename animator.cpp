...
void CAnimator::Uninit(void)
{
	//ベクトルをクリアする
	m_vParts.clear();
	m_vParts.shrink_to_fit();
	m_vAnimSpeed.clear();
	m_vAnimSpeed.shrink_to_fit();
}
...
{
	if (m_nPresentAnim != nPresentAnim && nPresentAnim < (int)m_vAnimAll.data()[m_type].vAnimSets.size())
	{
