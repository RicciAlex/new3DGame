...
HRESULT CFirstStage::Init(void)
{
	...
	/*const char str[CPauseMenu::TYPE_MAX][48] = { "Continue", "Quit" };

	for (int nCnt = 0; nCnt < CPauseMenu::TYPE_MAX; nCnt++)
	{
		D3DXVECTOR3 pos = D3DXVECTOR3((float)SCREEN_WIDTH * 0.5f, (float)SCREEN_HEIGHT * 0.5f, 0.0f);
		pos.y += (-100.0f * (CPauseMenu::TYPE_MAX * 0.5f)) + (100.0f * nCnt);

		CMenuButton::Create(pos, D3DXVECTOR2(250.0f, 100.0f), str[nCnt]);
	}*/
	...
	CStalkingBot* pBot = CStalkingBot::Create(D3DXVECTOR3(-1300.0f, -170.0f, -1250.0f), -199.9f);
	pSpwnTrigger->AddObject(pBot, -170.0f);

	//CCheckpoint* pCPoint = CCheckpoint::Create(const D3DXVECTOR3(D3DXVECTOR3(0.0f, -200.0f, 0.0f)));
	//pSpwnTrigger->AddObject(pCPoint, -199.9f);

	CGoldStar* pStar = CGoldStar::Create(D3DXVECTOR3(-2400.0f, -199.9f, -1650.0f));
	pSpwnTrigger->AddObject(pStar, -175.0f);
	...
	CGuideArrow::Create(D3DXVECTOR3(800.0f, 120.1f, -125.0f));
